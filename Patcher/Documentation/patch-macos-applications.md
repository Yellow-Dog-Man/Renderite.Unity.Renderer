...
6. (Optional) Check **Skip Signing** to prevent the patched application from being automatically signed.
   1. **Note:** When signing is skipped, the patched application will need to be signed manually.
...
> **Note**: Some macOS applications can not be properly signed by the Unity Application Patcher. For example, an application configuration might depend on an appropriate signing identity, or an application might have a file structure which is not expected by the Unity Application Patcher signing process. When trying to patch such an application, the signing process will be aborted, and a corresponding warning will be added to the log, informing the developer to sign the application manually. In cases when the developer intends to sign the application manually after patching, or when automatic signing is not possible, the automatic signing process can be skipped using either the **Skip Signing** checkbox or the **-skipSigning** command line flag.
...
UnityApplicationPatcherCLI -macos -applicationPath <path/to/my/application.app> -skipSigning
...
UnityApplicationPatcherCLI -macos -unityPlayerLibrary <path/to/my/application/UnityPlayer.dylib> -skipSigning
UnityApplicationPatcherCLI -macos -unityPlayerLibrary <path/to/my/application/UnityPlayer.dylib> -allowStandaloneLibrary
```
...
UnityApplicationPatcher.app/Contents/MacOS/UnityApplicationPatcherCLI -macos -applicationPath <path/to/my/application.app> -skipSigning
...
UnityApplicationPatcher.app/Contents/MacOS/UnityApplicationPatcherCLI -macos -unityPlayerLibrary <path/to/my/application/UnityPlayer.dylib> -skipSigning
UnityApplicationPatcher.app/Contents/MacOS/UnityApplicationPatcherCLI -macos -unityPlayerLibrary <path/to/my/application/UnityPlayer.dylib> -allowStandaloneLibrary
```
