...
6. (任意) パッチを適用したアプリケーションが自動的に署名されないようにするには、**署名をスキップ** をオンにします。
   1. **注意: **署名をスキップすると、パッチを適用したアプリケーションに手動で署名することが必要になります。
7. **パッチ** ボタンを押します。

> **注意**: Unity バージョン `2018.2` 以前でビルドされた Unity アプリケーションには、個別の `UnityPlayer.dylib` は含まれていません。この場合は、アプリケーションバンドル (`.app`) を選択します。

> **注意**: Unity アプリケーションパッチャーで macOS アプリケーションに適切に署名できない場合があります。例えば、アプリケーションの設定が適切な署名 ID に依存している場合や、アプリケーションのファイル構造が Unity アプリケーションパッチャーの署名プロセスで想定されていない場合です。このようなアプリケーションにパッチを適用しようとすると、署名プロセスは中止され、対応する警告がログに加えられて、開発者に手動でアプリケーションに署名するよう通知します。開発者がパッチ適用後に手動でアプリケーションに署名する予定の場合、または自動的な署名が可能でない場合には、**署名をスキップ** チェックボックスまたは **-skipSigning** コマンドラインフラグを使用して、自動署名プロセスをスキップできます。
