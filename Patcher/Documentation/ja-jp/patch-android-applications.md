...
| [よくあるご質問](#faq) | Android アプリケーションへのパッチ適用に役立つ、よくある質問への回答を紹介します。 |
...
| **バージョンコード** (入力必須項目) | Android APK/AAB のバージョンコード。ターゲットアプリケーションの現在のバージョンコードよりも大きくなければなりません。0 を指定すると、現在のバージョンコードが維持されます。(**注意**: Unity エディターやエンジンのバージョンではありません)。 |
...
Linux: 
1. パッチを適用する Unity アプリケーションがあるフォルダーに移動します。
2. ログインしているユーザーが、そのアプリケーションのファイルとフォルダーの読み取りと書き込みができることを確認します。
