...
6. Complete the **Application Version** information related to your Android application's APK/AAB and **Signing Information** (if necessary):
   1. **Version Code** (Required field):  Enter a new version code. This must be greater than the current version of the target `.apk` or `.aab`. If 0 is specified, the current version code will be kept.
   2. **Keystore**: Use the **Browse** button to locate the `.keystore` or `.jks` file for this application, and select **Open**. See the [note below](#sign-patched-application) for information about leaving this field empty.
   3. **Keystore Password**: Enter the password for the selected `.keystore` or `.jks` file.
   4. **Key Alias**: Enter the key alias for the selected `.keystore` or `.jks` file.
   5. **Key Alias Password**: Enter the password for the provided key alias.
7. Press the **Patch** button.

> **Note**: The **Version Code** field refers to your Android APK/AAB version code, not your Unity project or Unity Editor version.

> **Note**: You can click the **Query Version** button to acquire application's current **Version Code**.
...
| **Version Code** (Required) | The Android APK/AAB version code. Must be greater than the current version code of your target application. If 0 is specified, the current version code will be kept. (**Note**: Not the Unity Editor or engine version.) |
...
> **Note**: Use the `-verbose` or `-v` arg for a more detailed log of the patching process.
...
On Linux:
1. Navigate to the folder where the Unity application you want to patch resides.
2. Ensure that the files and folders of the application are readable and writable by the user you are logged in as.
...
If the current version code is unknown, use the **Query Version** button in UI version of this tool, or use the optional `-printApplicationVersion` arg in the CLI version of this tool display the current version code of the target `.apk` or `.aab`.

> **Note**: To keep the current version code of the target `.apk` or `.aab`, specify `0` in the version code field.
