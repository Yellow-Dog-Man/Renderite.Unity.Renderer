...
> **참고**: Unity 2017.1 이하 버전으로 빌드된 Unity 애플리케이션에는 별도의 `UnityPlayer.dll`이 포함되어 있지 않습니다. 이 경우에는 애플리케이션 폴더나 기본 `.exe` 파일을 선택합니다.

> **참고**: 여러 아키텍처에서 애플리케이션을 패치할 경우, 애플리케이션 폴더의 경로를 **애플리케이션 DLL 경로** 필드에 붙여 넣으면 여러 `UnityPlayer.dll`을 단일 배치로 패치할 수 있습니다. 여기서 예상되는 폴더 구조는 `<application/architecture/config/UnityPlayer.dll>`입니다.

![Windows용 Unity 애플리케이션 패치 프로그램.](../images/unity-application-patcher-windows-kr.png)<br/>*Windows 애플리케이션 패치용 툴*
...
```shell
./UnityApplicationPatcher.CLI -windows -unityPlayerLibrary <path/to/my/UnityPlayer.dll>
./UnityApplicationPatcher.CLI -windows -applicationPath <path/to/my/application>
```
