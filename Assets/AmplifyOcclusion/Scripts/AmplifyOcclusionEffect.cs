...
namespace AmplifyOcclusion
{
	...
	public class AmplifyOcclusionEffect : MonoBehaviour
	{
		...
		[SerializeField] private Shader m_applyShader;
		...
		static private Material m_applyMat = null;

		private void checkMaterials( bool aThroughErrorMsg )
		{
			if (m_occlusionShader == null)
				m_occlusionShader = Shader.Find("Hidden/Amplify Occlusion/Occlusion");

            if (m_blurShader == null)
                m_blurShader = Shader.Find("Hidden/Amplify Occlusion/Blur");

            if (m_applyShader == null)
                m_applyShader = Shader.Find("Hidden/Amplify Occlusion/Apply");

            if ( m_occlusionMat == null )
			{
				m_occlusionMat = AmplifyOcclusionCommon.CreateMaterialFromShader( m_occlusionShader, aThroughErrorMsg );
			}

			if ( m_blurMat == null )
			{
				m_blurMat = AmplifyOcclusionCommon.CreateMaterialFromShader( m_blurShader, aThroughErrorMsg );
			}

			if ( m_applyMat == null )
			{
				m_applyMat = AmplifyOcclusionCommon.CreateMaterialFromShader( m_applyShader, aThroughErrorMsg );
			}
			...
			if ( m_applyMat != null )
			{
				if ( checkedforMRTBlendingFallback == false )
				{
					...
					useMRTBlendingFallback = m_applyMat.GetTag( "MRTBlending", false ).ToUpper() != "TRUE";
				}
			}
		}
		...
		void commandBuffer_FillApplyDeferred( CommandBuffer cb, bool logTarget )
		{
			...
			if ( !logTarget )
			{
				if ( UsingTemporalFilter )
				{
					...
					if ( UsingFilterDownsample == false )
					{
						...
						if ( useMRTBlendingFallback )
						{
							...
							PerformBlit( cb, m_applyMat, ShaderPass.ApplyPostEffectTemporal + getTemporalPass() ); // re-use ApplyPostEffectTemporal pass to apply without Blend to the RT.
						}
						else
						{
							...
							PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredTemporal + getTemporalPass() );
						}
						...
						if ( useMRTBlendingFallback )
						{
							...
							cb.SetRenderTarget( applyOcclusionTemporal, applyOcclusionTemporal[ 0 ] /* Not used, just to make Unity happy */ );
							PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredTemporalMultiply );
					...
					else
					{
						...
						PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredCombineFromTemporal );
					}
				...
				else
				{
					...
					// Multiply Occlusion
					cb.SetRenderTarget( m_applyDeferredTargets, m_applyDeferredTargets[ 0 ] /* Not used, just to make Unity happy */ );
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferred );
				}
			}
			else
			{
				...
				if ( UsingTemporalFilter )
				{
					...
					if ( UsingFilterDownsample == false )
					{
						...
						PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredLogTemporal + getTemporalPass() );
					}
					else
					{
						// UsingFilterDownsample == true
						...
						PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredLogCombineFromTemporal );
					}
				...
				else
				{
					...
					cb.SetRenderTarget( m_applyDeferredTargets_Log, m_applyDeferredTargets_Log[ 0 ] /* Not used, just to make Unity happy */ );
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyDeferredLog );
				}
		...
		void commandBuffer_FillApplyPostEffect( CommandBuffer cb )
		{
			...
			if ( UsingTemporalFilter )
			{
				...
				if ( UsingFilterDownsample == false )
				{
					...
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyPostEffectTemporal + getTemporalPass() );
					...
					if ( useMRTBlendingFallback )
					{
						...
						PerformBlit( cb, m_applyMat, ShaderPass.ApplyPostEffectTemporalMultiply );
				...
				else
				{
					...
					cb.SetGlobalTexture( PropertyID._AO_TemporalAccumm, temporalRTid );
					cb.SetRenderTarget( BuiltinRenderTextureType.CameraTarget );
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyCombineFromTemporal );
				}
			...
			else
			{
				...
				cb.SetRenderTarget( BuiltinRenderTextureType.CameraTarget );
				PerformBlit( cb, m_applyMat, ShaderPass.ApplyPostEffect );
			}
		...
		void commandBuffer_FillApplyDebug( CommandBuffer cb )
		{
			...
			if ( UsingTemporalFilter )
			{
				...
				if ( UsingFilterDownsample == false )
				{
					...
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyDebugTemporal + getTemporalPass() );
				}
				else
				{
					// UsingFilterDownsample == true
					...
					cb.SetGlobalTexture( PropertyID._AO_TemporalAccumm, temporalRTid );
					cb.SetRenderTarget( BuiltinRenderTextureType.CameraTarget );
					PerformBlit( cb, m_applyMat, ShaderPass.ApplyDebugCombineFromTemporal );
				}
			...
			else
			{
				...
				cb.SetRenderTarget( BuiltinRenderTextureType.CameraTarget );
				PerformBlit( cb, m_applyMat, ShaderPass.ApplyDebug );
			}
